"""
Hugging Face Spaces - Pure FastAPI Backend for Stock Price Prediction
This file is the entry point for HF Spaces deployment.
"""
...
app = FastAPI(
    ...
    description="LSTM-based stock price prediction backend service",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
<html>
<head>
    <title>Stock Price Predictor API</title>
    <style>
        * { box-sizing: border-box; }
        ...
        code { background: #16213e; padding: 4px 8px; border-radius: 4px; font-size: 14px; }
...
<body>
    <div class="container">
        ...
        <div class="input-group">
            <input type="text" id="ticker" placeholder="Enter ticker (e.g., AAPL, GOOGL, MSFT)" value="AAPL">
        ...
        <div class="api-info">
            ...
            <div class="endpoint">
                ...
                <code>/api/predict/{ticker}</code>
                <p>Get stock price prediction for any ticker symbol</p>
            </div>
...
const response = await fetch('/api/predict/AAPL');
const prediction = await response.json();
console.log(prediction);</pre>

            <pre>// cURL
curl https://your-space-name.hf.space/api/predict/AAPL</pre>

            <p style="margin-top: 20px;">
                <strong>Interactive API Docs:</strong>
                <a href="/docs" style="color: #00b894;">/docs</a> (Swagger UI) |
                <a href="/redoc" style="color: #00b894;">/redoc</a> (ReDoc)
            </p>
        </div>
    </div>
                    ...
                    result.innerHTML = `
                        <div class="result">
                            ...
                            <div class="metric" style="margin-top: 15px;">
                                ...
                                <div class="metric-value">${data.volume.toLocaleString()}</div>
                            </div>
...
async def api_predict(ticker: str):
    """
    Get stock price prediction for a ticker symbol

    - **ticker**: Stock ticker symbol (e.g., AAPL, GOOGL, MSFT)

    Returns next day's predicted Open, High, Low, Close, Volume with confidence score.
    """
